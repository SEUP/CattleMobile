<template>
	<Page class="page">
		<ActionBar class="bg-violet" title="ตรวจการกลับสัด">
		  ...
		  <ActionItem  @tap="gotoStep()"  ios.position="right">
			...
			<StackLayout>
					...
					<GridLayout class="txt-gr" columns="*, 2*" rows="2*, 3*">
							...
							<TextField class="gr-text" row="0" col="1" @tap="setDate(form,'reversal_date')" hint="โปรดกรอกข้อมูล" :text="`${ form.reversal_date ? $moment(form.reversal_date).format('DD MMMM YYYY') : 'โปรดกรอกข้อมูล'}`" />
						</GridLayout>

            <GridLayout class="txt-gr" columns="*, 2*" rows="2*, 3*">
							<StackLayout class="gr">
								<Label class="gr-label light" text="ผลตรวจ" row="0" col="0" />
							</StackLayout>
							<TextField class="gr-text" row="0" col="1"  @tap="setChoice(form,'reversal','ผลการตรวจการกลับสัด')" :text="`${getChoiceTextByID(form.reversal) || 'ไม่ระบุ'}`" />
						</GridLayout>
            
 	                    	<GridLayout v-if="form.reversal == '280200'" class="txt-gr" columns="*, 2*" rows="2*, 3*">
							<StackLayout class="gr">
								<Label class="gr-label light" text="วัน/เดือน/ปี" row="0" col="0" />
							</StackLayout>
							<TextField class="gr-text" row="0" col="1" @tap="setDate(form,'end_height')" hint="โปรดกรอกข้อมูล" :text="`${ form.end_height ? $moment(form.end_height).format('DD MMMM YYYY') : 'โปรดกรอกข้อมูล'}`" />
						</GridLayout>
...
export default {
    ...
    gotoStep: async function(){
        ...
        let cattle = await this.$store.dispatch("breeder_female/step1", this.form); 
         ...
         this.$router.push({ name:'step5',params: {cattle: this.data,form:cattle} })
    },
		load(){
