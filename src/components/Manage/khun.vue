			...
			<StackLayout>
				...
				<StackLayout class="card">
					<StackLayout class="card-menu bg-violet" orientation="vertical">
					 <Label text="เริ่มขุน" class="f30 dark" />
					...
					<StackLayout class="card">
					<StackLayout class="card-menu bg-violet" orientation="vertical">
					 <Label text="สิ้นสุดการขุน" class="f30 dark" />
						<GridLayout class="txt-gr" columns="*, 2*" rows="2*, 3*">
							<StackLayout class="gr">
								<Label class="gr-label light" text="วัน/เดือน/ปี" row="0" col="0" />
							</StackLayout>
...
export default {
	...
	methods:{
		...
		//	this.form.breeding_date = this.$moment(this.form.breeding_date ).format('YYYY-MM-DD')
			let farmer = await this.$store.dispatch("khun/update", this.form);
      if (farmer == 1) {
      this.read();
      }
		...
		read: async function(){
			let preData= await this.$store.dispatch("khun/read", this.data.id);
